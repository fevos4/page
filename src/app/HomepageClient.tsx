  ...
  const handlePlayClick = async (video: Video) => {
    ...
    try {
      ...
      if (data.source_type === 'embed') {
        setPlaySource({ source_type: 'embed', url: data.embed_url });
      } else {
        setPlaySource({ source_type: 'self_hosted', url: data.play_url });
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      ...
      >
        ...
        <div className="flex items-center justify-end min-w-[180px] space-x-4">
          <ThemeToggle />
      ...
      {/* 2. Hero Section: Full screen viewport height with Hero 2 image right-positioned */}
      <section className="relative w-full h-screen min-h-screen flex items-center justify-between overflow-hidden pt-20 px-6 md:px-16 text-slate-100 bg-slate-950">
        {/* Background Image (Hero 2.avif) positioned on the right side with reduced size */}
        <div
          className="absolute inset-y-0 right-0 w-full md:w-3/5 bg-contain bg-right bg-no-repeat z-0 transform filter brightness-95 opacity-90"
          style={{ backgroundImage: `url('/imgs/Hero 2.avif')` }}
        />
      ...
      <main id="browse-rows" className="flex-1 max-w-7xl w-full mx-auto p-6 md:p-12 space-y-12">
        ...
        {titles.map((title) => (
          <section key={title.id} className="space-y-4">
            ...
            {/* Horizontally Scrollable Row with Hover Expansion Spacing */}
            <div className="flex space-x-6 overflow-x-auto py-5 px-2 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-800">
              {title.videos.map((video) => {
                ...
                return (
                  <div
                    key={video.id}
                    className="flex-none w-72 group relative transition-transform duration-300 transform hover:scale-105 hover:z-30"
                  >
                    {/* Aspect Video Thumbnail with Netflix-style Expansion Zoom on Hover */}
                    <div className="relative aspect-video bg-slate-950 flex items-center justify-center overflow-hidden rounded-lg shadow-md group-hover:scale-110 group-hover:z-30 group-hover:shadow-2xl transition-all duration-300 transform origin-center">
                      <div
                      ...
                      {!isPlayable ? (
                        ...
                        >
                          <div className="flex justify-end">
                            <div className="w-10 h-10 rounded-full bg-white text-slate-950 flex items-center justify-center shadow-2xl transform group-hover:scale-110 transition-transform duration-300">
      ...
      {activeModalVideo && (
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative">
